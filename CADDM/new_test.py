...
def test():
    ...
    for batch_data, batch_labels, img_frame, ld in test_loader:
        ...
        # print("ld_new", ld)
        # Count scores
        less_than_0_10 = sum(1 for score in outputs if score < 0.10)
        greater_than_0_10 = sum(1 for score in outputs if score >= 0.10)

        print("op", outputs)
        print(f"Count of outputs < 0.10 (Real Frames): {less_than_0_10}")
        print(f"Count of outputs >= 0.10 (Fake Frames): {greater_than_0_10}")

        # print("ImgFS", img_frame.shape)
        # print("first_fdty", type(img_frame))
        # print("labelpr_dtype", type(outputs))
        # predicted_outputs = [f"{value:.3f}" for value in outputs]
        # print("LabelPr", predicted_outputs)

        # print("LabelPrlen", len(predicted_outputs))

        # print("outputs",labels.detach().cpu().numpy() )
        output_image = draw_landmarks_and_bbox_batch(img_frame, ld, outputs)

        # output_path3 = 'Image_Final_box.png'

        # print("out_t", type(output_image))
